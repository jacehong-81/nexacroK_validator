...
if (!nexacro.DesignForm)
{
        ...
        _pFormBase.loadForm = function (formurl, async, reload, baseurl)
        {
            if (this._load_manager)
            {
                ...
                if (this._p_parent._is_frame && this._p_parent._p_form == this)
                {
                    ...
                    nexacro.getApplication()._registerLoadforms(this);
                }
        ...
        if (__pLoadManager)
        {
            __pLoadManager.on_load_main = function (url, errstatus, module, fireerrorcode, returncode, requesturi, locationuri, extramsg)
            {
                if (url == this.main_url)
                {
                    ...
                    {
                        // adl 
                        if (this.context == nexacro.getApplication())
                        {
    ...
    {
        if (!nexacro.getApplication()._css_context_cache)
            return null;

        var ar = nexacro.getApplication()._css_context_cache;
    ...
    {
        if (!nexacro.getApplication()._css_context_cache)
            nexacro.getApplication()._css_context_cache = new nexacro.Collection();
    ...
    nexacro._removeDesignCssContext = function (url)
    {
        ...
        nexacro.getApplication()._css_context_cache.delete_item(url);
    };
    ...
    nexacro._updateDesignCssContext = function (cssurl)
    {
        ...
        nexacro.getApplication()._load_manager.localList = [];
        nexacro.getApplication()._load_manager.localCnt = 0;
    ...
    */
    ...
    {
        if (nexacro.getApplication().accessport)
        {
    ...
    _pDesignForm.getPrevComponent = function (targetcompid, rootcompid)
    {
        ...
        for (var i = comp_len - 1; i >= 0; i--)
        {
            ...
            if (comp == targetobj)
            {
                if (i > 0)
                {
                    return this._getScopeName(comps[i - 1]);
                }
                else
                {
                    break;
    ...
    _pDesignForm._getClassID = function (obj)
    {
        ...
        var registerclass = env._getRegisterClass(); //nexacro._registerclass;
        if (!registerclass) return obj._type_name;
    ...
    _pDesignForm.setLayoutProperty = function (compid, layoutname, propid, propval)//, currentlayoutname)
    {
        ...
        switch (propid)
        {
            ...
            case "type":
                if (fluidmanager)
                    obj._setInnerFlexibleFlag(manager.isFluidLayoutType(obj), fluidmanager.getLayoutArrangeType());
            //break;
            case "spacing":
            case "horizontalgap":
            case "vertcalgap":
    ...
    _pDesignForm.on_notify_init = function (obj, e)
    {
        ...
        var accessport = nexacro.getApplication().accessport.getFormAccessPort(this._url);
        if (accessport)
        {
    ...
    _pApplicationAccessPort.clearGlobalVariables = function ()
    {
        ...
        var datasets = nexacro.getApplication()._datasets;
    ...
    _pApplicationAccessPort.createFrame = function (classname, parentid, frameid)
    {
        ...
        if (classnameobj)
        {
            ...
            {
                // 이미 mainframe이 있는 경우?
                if (nexacro.getApplication()._p_mainframe)
                {
                    var mainframe = nexacro.getApplication()._p_mainframe;
                    mainframe._on_close();
                    mainframe._destroy();
                    delete nexacro.getApplication()[frameid];
                    nexacro.getApplication()._p_all.delete_item(frameid);
                    nexacro.getApplication()._p_mainframe = null;
                }

                obj = nexacro.getApplication().createMainFrame(frameid, null, null, null, null, null, null, null, parent);
    ...
    _pApplicationAccessPort.createInvisibleObject = function (classname, objid, parentid)
    {
        ...
        if (classnameobj)
        {
            var obj = new classnameobj(objid, nexacro.getApplication());
            if (obj instanceof nexacro.Dataset)
            {
                obj._setContents("");
                nexacro.getApplication()._addDataset(obj._p_name, obj);
            }

            else
            {
                nexacro.getApplication()._addObject(obj._p_name, obj);
            }
    ...
    _pApplicationAccessPort.deleteObject = function (objid)
    {
        ...
        if (app._datasets[objid])
        {
            var idx = app._datasets.indexOf(objid);
            app._datasets.splice(idx, 1);

            delete app[objid];
            app._p_all.delete_item(objid);
        }
        else
        {
            delete app[objid];
    ...
    _pApplicationAccessPort._getObject = function (objid)
    {
        ...
        // 140702 박현진 : frame 찾기
        var obj = eval("nexacro.getApplication()." + objid);
        if (obj)
            return obj;

        return nexacro.getApplication()._p_all.get_item(objid);
    };
    ...
    _pApplicationAccessPort.setCssList = function (csslist)
    {
        ...
        // ver2; Cache
        const app = nexacro.getApplication();
        app._css_context_list = [];
        app._find_csslist = []; // context._css_selectors의 array

        app._load_manager.localList = [];
        app._load_manager.localCnt = 0;
        ...
        for (var i = 0; i < csslist.length; i++)
        {
            ...
            app._css_context_list.push(css_context);
            app._find_csslist.unshift(css_context._css_selectors);
        }
    ...
    _pApplicationAccessPort._clearStyles = function (comp)
    {
        ...
        {
            app._find_csslist = null;
            app._cssfinder_cache = {};
    ...
    {
        if (comp === undefined)
            comp = nexacro.getApplication()._p_mainframe;
        else if (comp === null)
            return;
    ...
    nexacro.createApplicationAccessPort = function ()
    {
        const app = nexacro.getApplication();
        if (!app.accessport)
            app.accessport = new nexacro.ApplicationAccessPort(app);
    };

    nexacro.destroyApplicationAccessPort = function ()
    {
        const app = nexacro.getApplication();
        var app_ap = app.accessport;
        if (app_ap)
        {
    ...
    nexacro.closeApplication = function ()
    {
        ...
        if (app)
        {
            var mainframe = app._p_mainframe;
            if (mainframe)
                mainframe._destroy();
