...
if (!nexacro.CellInfo)
{
    ...
    _pCellInfo._property_map_maskedit = [
        ...
        ["maskedittrimtype", "trimtype", true, "none", false, 2, ["none", "left", "righ", "both"]],
    ...
    _pCellInfo._property_map_combo = [
		...
		["combopopuptype", null, true, "normal", false, 2, ["normal", "center", "system", "none"]],
    ...
    _pCellInfo._property_map_calendar = [
        ...
        ["calendarbackgroundcolumn", "backgroundcolumn", true, "", false],
        ["calendarbordercolumn", "bordercolumn", true, "", false],
        ["calendardatecolumn", "datecolumn", true, "", false],
        ["calendartextcolorcolumn", "textcolorcolumn", true, "", false],
    ...
    {
        var prop, bind, deft, str, type, styl, eval_str = "";
    ...
    _pCellInfo.__parseDate = function (v, dFlag, rowidx)
    {
        ...
        if (dFlag > 0)
        {
            ...
            if (dFlag == 1)
            {
				...
				var milsec = v.length == 17 ? +v.substring(14, 17) : 0;
            }
            else
            {
                var hour = 0;
        ...
        else
        {
			...
			var milsec = v.length == 9 ? +v.substring(6, 9) : 0;
        }
...
if (!nexacro._CellControl)
{
        ...
        if (text_elem)
        {
            ...
            else
            {
                ...
                if (!text_elem.text && strtext)
                {
                    ...
                    {
                        this._expandCtrl.destroy();
                        this._createExpandDisplayer(); // node 순서때문
    ...
    _pCellControl.__update = function (status, onlycontents, nochk_updatearea)
    {
        ...
        if (this.parent._rowidx >= 0)
        {
            if (this._rowidx != this.parent._rowidx)    // parent는 변경됬는데 update가 안됬을 경우
            {
                ...
                this._rowidx = this.parent._rowidx; // parent : cell or viewrow
    ...
    _pCellControl._updateAvailableArea = function (force)
    {
        ...
        if (subcomp)
        {
            if (this._curDisplayType == "checkboxcontrol")
                this.on_apply_checkboxAlign("center", "middle");    // 현재는 중앙 고정값으로 사용.
            else
            {
    ...
    _pCellControl._destroyEditor = function ()
    {
        if (this._editor)
        {
            ...
            {   
                ...
                if (control_elem)
                    control_elem.setElementDisplay("none"); //rp84592
            }
    ...
    _pCellControl._createEditor_button = function ()
    {
        ...
        {   // view edit가 존재시 다른 컴포넌트로 focus가 나갔다가 들어오면 버튼 생성 전 기존 edit로 focus가 돌아가는 현상이 있음.
            nexacro._OnceCallbackTimer.callonce(this, function ()
            {
    ...
    _pCellEdit._setAccessibilityStatFocus = function ()
    {
        ...
        if (!this._displaymode)
            this._input_element._setAccessibilityLabel(this._getAccessibilityReadLabel(false));
        else
            this._control_element.setElementAccessibilityLabel(cellobj.accessibilitylabel);
    ...
    _pCellCalendar._getDlgCode = function (keycode, altKey, ctrlKey, shiftKey)
    {
        ...
        return { want_tab: (this._view ? this._view._acceptstab : false), want_return: false, want_escape: false, want_chars: false, want_arrows: _want_arrows };
    };
