...
if (!nexacro.Component)
{
    ...
    _pComponent._initContents = function (/*control_elem*/)
    {
    ...
    _pComponent.set_fittocontents = function (v)
    {
        ...
        if (this.fittocontents != v)
        {
            ...
            if (this._is_created)
            {
                if (this.fittocontents == "height") // for wordwrap (orginal width)
                {
    ...
    _pComponent.setFocus = function (bResetScroll, bInnerFocus)
    {
        ...
        if (nexacro._enableaccessibility)
        {
            if (!this._isEnable())
            {
                if (focus_direction != 2 && focus_direction != 3)       // KEYUP, KEYDOWN
                    return;
            }
            // 미리 체크할 항목: visiblestate / enablestate / focusacceptable
            if (!this._isVisible() /* || !this.enableevent */ || !this._isFocusAcceptable())
                return;
        }
        else
        {
        ...
        if (bResetScroll)
        {
            ...
            {
                if (this.parent._bFireLoadEvent == true) //form.onloading
                {
    ...
    */
    ...
    _pComponent.on_apply_status = function (/*status, userstatus, is_userstatus, status_param, value_param*/) { };
        ...
        else
        {
            if (info && (obj = info.width))
            {
                if (form && form._is_created_contents)
                {
                    ...
                    this._width = this._convToPixel(obj.distance, comp_width);//comp._getClientWidth());
                }
            }
            else
            {
        ...
        else
        {
            if (info && (obj = info.height))
            {
                if (form && form._is_created_contents)
                {
                    ...
                    this._height = this._convToPixel(obj.distance, comp_height);//comp._getClientHeight());
                }
            }
            else
            {
    ...
    _pComponent._getCSSStyleValue = function (prop, status, userstatus)
    {
        ...
        if (userstatus && this._cssselector[userstatus])
        {
            ...
            //trace("debug : " + value + " " + bRtl + " " + rtlvalue);
    ...
    /* 
        _pComponent._getComputedStyles = function ()
        {        
            //{background : strvalue, 
            //    border : strvalue, 
            //    padding : strvalue}         
        ...
        _pComponent._getComputedStyle = function (prop)
        {   
            if (prop)
            {
                return value;
    ...
    */
    ...
    //_pComponent._getElementClassCSSSelector = _pComponent._getClassCSSSelector;
    ...
    */
    ...
    // Position will determine it's parents rtl property value.
            ...
            else
            {
                if (layer_info.ref_first_modal_frame)
                {
                    ...
                    if (win.dest_handle && win.dest_handle._linked_element)
                        ret.owner_handle = win.dest_handle._linked_element.dest_handle;  //이때 win.dest_handle._linked_element 는 Mainframe ControlElelement && ret.owner_handle은 simplecontainerelement
                    else
                ...
                else
                {
                    ...
                    /*
                    if (isNaN(win.dest_handle))
                    {
                        trace("isNaN(win.dest_handle)==true");
                        ret.owner_handle = win.dest_handle;
                        if (win.dest_handle._linked_element)
                            ret.ref_handle = win.dest_handle._linked_element.dest_handle;
                        ret._is_append = true;
                    }
                    else // runtime은 handle이 number
                    {
                        trace("nexacro.getApplication().mainframe._control_element.dest_handle:" + nexacro.getApplication().mainframe._control_element.handle);
                        ret.owner_handle = nexacro.getApplication().mainframe._control_element.handle;
                        ret._is_append = true;
                    }
                    */
                }
