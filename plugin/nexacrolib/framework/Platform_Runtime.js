...
if (nexacro._Browser == "Runtime")
{
	if (!nexacro._init_platform_runtime)
	{
		...
		nexacro._SupportTouchEvent = true; // for event test
		...
		{
			let popupframes, window;
			if (!parentframe)
				window = nexacro._findWindow(nexacro._getMainWindowHandle());
			else
				window = _application._parentwindow;
		...
		nexacro._unregisterPopupFrame = function (id, winobj, usehtmlonly/*do not use runtime*/, use_argwin)
		{
		...
		nexacro._syshandler_onlbuttonup_forward = function (_window, elem, strButton, altKey, ctrlKey, shiftKey, windowX, windowY, screenX, screenY, metaKey)
		{
			...
			if (elem && elem._use_decoration)
			{
				...
				if (firecur_comp)
				{
					...
					ret = nexacro.__hitTestByDecorateText(elem.handle, clientXY[0], clientXY[1],
						function (v)
						{
							if (v)
							{
		...
		nexacro._syshandler_onmousewheel_forward = function (_window, elem, wheelDelta, strButton, altKey, ctrlKey, shiftKey, windowX, windowY, screenX, screenY, metaKey)
		{
			...
			if (nexacro._nCountAlertConfirm <= 0)
			{
				...
				if (ret != false && ctrlKey && wheelDelta != 0)
				{
					...
					var zoomframeElement = _window.root._is_popup_frame != true ? (_window.root.frame ? _window.root.frame.getElement() : null) : _window.root.getElement();
					if (!zoomframeElement) return ret;
		...
		{
			//trace("nexacro._syshandler_ondragenter_forward : ", _window, elem, strButton, altKey, ctrlKey, shiftKey, windowX, windowY, screenX, screenY, filelist)
		...
		{
			//trace("nexacro._syshandler_ondragleave_forward : ", _window, elem, toElem, strButton, altKey, ctrlKey, shiftKey, windowX, windowY, screenX, screenY)
		...
		{
			//trace("nexacro._syshandler_ondragover_forward : ", _window, elem, strButton, altKey, ctrlKey, shiftKey, windowX, windowY, screenX, screenY)
		...
		{
			//trace("nexacro._syshandler_ondrop_forward : ", _window, elem, strButton, altKey, ctrlKey, shiftKey, windowX, windowY, screenX, screenY)
		...
		nexacro._syshandler_ontouchend_forward = function (_window, elem, touchid, time/*, windowX, windowY, screenX, screenY, is_last_changedtouch*/)
		{
			...
			if (elem && elem._use_decoration)
			{
				...
				if (firecur_comp)
				{
					...
					ret = nexacro.__hitTestByDecorateText(elem.handle, clientXY[0], clientXY[1]/*- 5*/,
						function (v) { if (v) { nexacro._execBrowser(v); } });
				}
			}
		...
		nexacro._syshandler_onmousehover_forward = function (_window, elem, strButton, altKey, ctrlKey, shiftKey, windowX, windowY/*, screenX, screenY*/, metaKey)
		{
			if (elem && elem.handle)
		...
		nexacro._syshandler_oncontextmenu_forward = function (_window, elem, strButton, altKey, ctrlKey, shiftKey, windowX, windowY, screenX, screenY, metaKey)
		{
			var clientXY, clientX, clientY;
			var firecur_comp = _window._findComponentForEvent(elem);
			if (firecur_comp)
			{
				var elem_pos = nexacro._getElementXYInWindow(firecur_comp._control_element.handle);
			...
			return _window._on_sys_contextmenu(elem, "contextmenu", altKey, ctrlKey, shiftKey, windowX, windowY, screenX, screenY, clientX, clientY, metaKey);
		};
		...
		nexacro._syshandler_onbeforeclose_forward = function (_window)
		{
			...
			{
				return nexacro._confirm(_window.root, confirm_message);
			}
		...
		nexacro._syshandler_onviewsource_forward = function (_window, elem)
		{
			if (elem)
			{
				...
				if (comp)
				{
					...
					if (ref_form)
					{
						var owner = ref_form._p_parent;
						if (comp._p_url)
						{
							formurl = comp._p_url;
						}
						else if (ref_form._is_frame) // childframe
						{
							formurl = ref_form._p_formurl;
						}
						else if (owner._is_frame) // childframe
						{
							formurl = owner._p_formurl;
						}
						else
						{
							if (ref_form._url)
							{
								...
								formurl = "./" + formurl.substr(nexacro._project_absolutepath.length);
							}
						}
		...
		{
			//trace("_syshandler_onextmodule_forward handle - ", handle);
			_win._on_sys_onextmodule(handle, eventinfoarray);
		};

		/*
		nexacro._syshandler_onextendedcommand_forward = function (_win, evt)									
		{
			//trace("_syshandler_onextendedcommand_forward");         
			_win._on_sys_onextendedcommand(evt);
		};
		*/
		...
		nexacro.__bindEventPopupWindowLoadHandler = function (_window, parent_win, popupframeoptions)
		{
			return function (v1, v2)
			{
				if (_window == null) _window = v1;
				if (_window)
				{
					...
					childframe._popupframeoption = popupframeoptions;
		...
		nexacro._createWindowHandle = function (parent_win, _window, name, left, top, width, height, resizable, layered, taskbar, is_main)
		{
			...
			if (is_main && handle)
			{
				nexacro._isRunBaseWindow(_window);   // modal, open 시 nexacro._registerPopupFrame 관리 위한 window 설정
		...
		nexacro._createModalWindowHandle = function (parent_win, _window, name, left, top, width, height, resizable, layered, lockmode)
		{
			...
			while (root_win._p_parent)
				root_win = root_win._p_parent;
			...
			if (lockmode == 1) // lock=all
			{
				...
				if (popupframes)
				{
					...
					for (i = 0; i < len; i++)
					{
						...
						while (popup_root._p_parent)
							popup_root = popup_root._p_parent;
			...
			//modal 이라 이미 destroyed handle.. invalid handle 넣으면 오동작 //nexacro.__setWindowHandleLock(handle, false, handle, false);
			//window destroyed 시 내부에서 unlock 처리하므로 여기선 별도 처리 거치지 않음.

			popup_root_win_list = null;
		...
		nexacro._createOpenWindowHandle = function (parent_win, name, formurl, left, top, width, height, resizable, layered, taskbar, is_main, parentframe, frameopener, arr_arg, ext_options, openstyles)
		{
			...
			//parent 가 null 일때 popupframe에서 삭제할수 있는 방법이 없어 _parentwindowforopen 를 설정함   
			//if (parent_win)
			//    _window._parentwindowforopen = parent_win;
			//else
			_window._parentwindowforopen = nexacro._findWindow(nexacro._getMainWindowHandle());

			var callback = nexacro.__bindEventPopupWindowLoadHandler(_window, parent_win, popupframeoptions);
		...
		nexacro._isPopupWindowApp = function ()   // for html
		{
			return false;
		};


		nexacro._ishandleWindow = function (handle)
		{
			return true;
		}
		nexacro._handletoWindow = function (handle)
		{
			return handle;
		};
		...
		nexacro._getWindowHwndHandle = function (_handle) //get nre hwnd
		{
			if (_handle)
			{
		...
		nexacro._getContainerHandlePosX = function (handle)
		{
			return nexacro.__getWindowHandlePosX(handle);
		}
		nexacro._getContainerHandlePosY = function (handle)
		{
			return nexacro.__getWindowHandlePosY(handle);
		}
		...
		nexacro._getContainerHandleClientWidth = function (handle)
		{
			return nexacro.__getWindowHandleClientWidth(handle);
		}
		nexacro._getContainerHandleClientHeight = function (handle)
		{
			return nexacro.__getWindowHandleClientHeight(handle);
		}

		nexacro._getWindowHandleText = nexacro._emptyFn;
		...
		nexacro._getSharedVariable = function (name, type)
		{
			let env = nexacro.getEnvironment();
			if (env && env._engineshared && env._engineshared[type])
			{
				return env._engineshared[type][name]?.value;
			}
			return undefined;
		};

		nexacro._setSharedVariable = function (name, value, type)
		{
			let env = nexacro.getEnvironment();
			if (env)
			{
				if (!env._engineshared) env._engineshared = {};
				if (!env._engineshared[type]) env._engineshared[type] = {};
				...
				env._engineshared[type][name] = { "type": vartype, "value": value };
			}
		};

		nexacro._removeSharedVariable = function (name, type)
		{
			let env = nexacro.getEnvironment();
			if (env && env._engineshared && env._engineshared[type])
			{
				env._engineshared[type][name] = null;
				delete env._engineshared[type][name];
			}
		};
		...
		nexacro._getSharedVariableAll = function (type)
		{
			...
			if (env && env._engineshared && env._engineshared[type])
			{
				return env._engineshared[type];
			}
		};

		nexacro._hasSharedVariable = function (name, type)
		{
			let env = nexacro.getEnvironment();
			if (env && env._engineshared && env._engineshared[type])
			{
				const value = env._engineshared[type][name];
				if (value)
				{
					return value.type ? true : false;  //변수가 있고 type이 undefined 일 경우 type이 있다
				}
			}
			return false;
		};
		...
		{
			let env = nexacro.getEnvironment();
			if (env?._engineshared)
				env._engineshared = null;
		};

		// type 삭제 : privateprofile 만 localstorage 사용
		nexacro._removePrivateProfile = function (key, global)
		{
			nexacro.__removePrivateProfile(key, global);
		};
		...
		nexacro._writeTraceLog = function (msglevel, message, bsystemlog, loglevel, traceduration, tracemode)
		{
			...
			nexacro.__writeTraceLog(data, loglevel || 4, tracemode || "none", traceduration || -1, msglevel);
		...
		*/
		...
		nexacro._setViewportScale = function (_window)
		{
			...
			var use_autozoom = (env._zoom_factor === 0 ? false : true);
			var ratio = (use_autozoom ? env._zoom_factor / 100 : 1.0);
			...
			_window._zoom_factor = env._zoom_factor;
			if (env._zoom_factor !== 0 && isNaN(env._zoom_factor) == false)
			{
				_window._zoom_factor = env._zoom_factor;
				nexacro.__setWindowHandleZoom(handle, env._zoom_factor);
			}
		};
		...
		nexacro._syshandler_ontray_forward = function (_window, type, id, button, alt_key, ctrl_key, shift_key, screenX, screenY, meta_key, hidereason)
		{
			...
			else if (type == "rbuttonup")
			{
				...
				{
					tray = application._p_trays[id];
					if (tray) tray.on_fire_onrbuttonup(button, alt_key, ctrl_key, shift_key, screenX, screenY, meta_key);
				}
			}
			else if (type == "dblclick")
			{
				...
				{
					tray = application._p_trays[id];
					if (tray) tray.on_fire_ondblclick(button, alt_key, ctrl_key, shift_key, screenX, screenY, meta_key);
				}
			}
			else if (type == "menuclick")
			{
				if (nexacro._current_tray_popup)
				{
					...
					if (traypopup) traypopup.on_fire_onmenuclick(id);
				}
			}
			else if (type == "balloontipshow")
			{
				...
				{
					tray = application._p_trays[id];
					if (tray) tray.on_fire_onballoontipshow();
				}
			}
			else if (type == "balloontiphide")
			{
				...
				{
					tray = application._p_trays[id];
					if (tray) tray.on_fire_onballoontiphide(hidereason);
				}
			}
			else if (type == "balloontipclick")
			{
				...
				{
					tray = application._p_trays[id];
					if (tray) tray.on_fire_onballoontipclick(button, alt_key, ctrl_key, shift_key, screenX, screenY, meta_key);
				}
			}
		...
		{
			if (layered) frame.set_background("transparent");
		};

		nexacro._postMessage = function (id, _window/*, target_comp, targetorigin*/)
		{
		...
		nexacro._preventBackNavigation = nexacro._emptyFn;
		nexacro._isPageReloadKey = nexacro._emptyFn;


		nexacro._loadExtCss = async function (item)
		{
			return Promise.reject(new Error("Unsupported environment: Runtime"));
		};
		...
		nexacro.History = (function ()
		{
			...
			historymodule.pushState = nexacro._emptyFn;
			historymodule.replaceCurrentState = nexacro._emptyFn;
			historymodule.getCurrentState = nexacro._emptyFn;
			historymodule.setCurrentStateData = nexacro._emptyFn;
			historymodule.getCurrentStateData = nexacro._emptyFn;
			historymodule.back = nexacro._emptyFn;
			historymodule.forward = nexacro._emptyFn;
